...
from (
    ...
    select query
        ...
        ,case when create_date = date_sub(current_date,1) then '1' end is_t1
        ,houseid house_id 
    ...
    from (
        ...
        and status = '80' -- 已发布
        -- and tiktok_channel_code = 957780
    ) a 
...
left join (
    ...
    select  
        ...
        ,t1.pro_channel
        ,nvl(t1.channel,'other') is_new
        ,count(distinct concat(t2.dt,t2.uid)) as uv
        ,count(distinct case when t2.order_time is not null then concat(to_date(t2.order_time),t4.uid) end) as yd_uv
        ,sum(t4.order_num) as yd_ord
        ,sum(t4.night) as yd_jianye
        ,sum(t4.gmv) as yd_gmv
        ,count(distinct case when t2.order_time is not null then concat(to_date(t2.order_time),t3.uid) end) as ld_uv
