with o as (
SELECT house_id,count(uid) pv,avg(final_price) as final_price
...
--and city_id in (select city_id from excel_upload.wanghong_29_city_1017 group by 1)
--and city_id in (select house_city_id from h group by 1)  --国内
and client_name = 'APP' 
and user_type = '用户'
group by 1
),
...
select dt,house_id,hotel_id,house_city_id,dynamic_business_id,house_city_name
                ,longitdws_path_ldbo_dude,latitude
                ,bed_info,share_type
                ,great_tag
