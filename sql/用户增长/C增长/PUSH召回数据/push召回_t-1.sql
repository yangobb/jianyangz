...
select
    ...
    ,max(case when (detail_uid is not null) then 1 end) detail_page
...
select user_id uid 
    ...
    ,case when c.uid is not null and (pmod(dayofweek(checkindate) + 5, 7) + 1) in (6,7) then 'bj001' -- 抹零卡 d.uid 取消
        ...
        when checkindate = current_date and checkoutdate = date_add(current_date,1) then 'cg013'
        when checkoutdate >= current_date then 'cg014'
        else 'yhysj'
        end scene
