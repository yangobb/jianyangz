...
select  dt
    ,uid
    ,sum(hotel_od_cnt) hotel_od_cnt_all
    ,sum(case when adr_type = '100-' then hotel_od_cnt end) hotel_od_cnt_100
    ,sum(case when adr_type = '100-300' then hotel_od_cnt end) hotel_od_cnt_300
    ,sum(case when adr_type = '300-500' then hotel_od_cnt end) hotel_od_cnt_500
    ,sum(case when adr_type = '500+' then hotel_od_cnt end) hotel_od_cnt_501
from (
    select
        to_date(orderdate) as dt
        ,clientid as uid
        ,case when ciireceivable / ciiquantity < 100 then '100-'
            when ciireceivable / ciiquantity between 100 and 300 then '100-300'
            when ciireceivable / ciiquantity between 300 and 500 then '300-500'
            when ciireceivable / ciiquantity >= 500 then '500+'
            else '其他' end adr_type  
    ...
    and to_date(orderdate) >= '2025-05-07'
...
select dt 
    ,uid 
    ,sum(ms_od_cnt) ms_od_cnt_all
    ,sum(case when adr_type = '100-' then ms_od_cnt end) ms_od_cnt_100
    ,sum(case when adr_type = '100-300' then ms_od_cnt end) ms_od_cnt_300
    ,sum(case when adr_type = '300-500' then ms_od_cnt end) ms_od_cnt_500
    ,sum(case when adr_type = '500+' then ms_od_cnt end) ms_od_cnt_501
from (
    select to_date(create_date) dt 
        ,uid 
        ,case when ld_gmv / order_room_night_count < 100 then '100-'
            when ld_gmv / order_room_night_count between 100 and 300 then '100-300'
            when ld_gmv / order_room_night_count between 300 and 500 then '300-500'
            when ld_gmv / order_room_night_count >= 500 then '500+'
            else '其他' end adr_type  
        ,count(distinct order_no) ms_od_cnt 
        ,sum(order_room_night_count) ms_od_night	
        ,sum(ld_gmv) ms_od_gmv -- real_pay_amount
...
,cross_info as (
select cross_uid.city_name
    ,cross_uid.city_id
    ,cross_uid.geo_position_id
    ,cross_uid.uid
    ,cross_uid.dt
    ,hotel_od_cnt_all
    ,hotel_od_cnt_100
...
select 
    ...
    ,case when cansale_num_l21plus_zc<50 then 'L21+直采可售数量小于20' else 'L21+直采可售数量高于20' end as type2
...
left join cross_info
on ldbo.city_id = cross_info.city_id
and ldbo.city_name = cross_info.city_name
and ldbo.geo_position_id = cross_info.geo_position_id
)
...
select 
    ...
    ,sum(case when bucket = '实验' then ms_od_cnt_all end) / sum(case when bucket = '实验' then hotel_od_cnt_all end) `交叉渗透率_实验`
    ,sum(case when bucket = '对照' then ms_od_cnt_all end) / sum(case when bucket = '对照' then hotel_od_cnt_all end) `交叉渗透率_对照`

    ,sum(case when bucket = '实验' then ms_od_cnt_all end) / sum(case when bucket = '实验' then hotel_od_cnt_100 end) `100以下交叉渗透率_实验`
    ,sum(case when bucket = '对照' then ms_od_cnt_all end) / sum(case when bucket = '对照' then hotel_od_cnt_100 end) `100以下交叉渗透率_对照`
    ,sum(case when bucket = '实验' then ms_od_cnt_all end) / sum(case when bucket = '实验' then hotel_od_cnt_300 end) `100到300交叉渗透率_实验`
    ,sum(case when bucket = '对照' then ms_od_cnt_all end) / sum(case when bucket = '对照' then hotel_od_cnt_300 end) `100到300交叉渗透率_对照`
    ,sum(case when bucket = '实验' then ms_od_cnt_all end) / sum(case when bucket = '实验' then hotel_od_cnt_500 end) `300到500交叉渗透率_实验`
    ,sum(case when bucket = '对照' then ms_od_cnt_all end) / sum(case when bucket = '对照' then hotel_od_cnt_500 end) `300到500交叉渗透率_对照`
    ,sum(case when bucket = '实验' then ms_od_cnt_all end) / sum(case when bucket = '实验' then hotel_od_cnt_501 end) `500以上交叉渗透率_实验`
    ,sum(case when bucket = '对照' then ms_od_cnt_all end) / sum(case when bucket = '对照' then hotel_od_cnt_501 end) `500以上交叉渗透率_对照`
