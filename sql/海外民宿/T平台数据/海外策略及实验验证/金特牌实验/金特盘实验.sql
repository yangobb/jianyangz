...
,list as (
select a.*
    ,case when t2.bucket_type in ('C','D','E') then '金特牌实验'
        when t2.bucket_type in ('I','J','K') then '宝藏实验'
        when t2.bucket_type in ('F','G','H') then '对照'
        else '其他'
        end as bucket
    ,check_type 
    ,t2.bucket_type
...
from (
    ...
    where dt between '2025-07-03' and date_sub(current_date,1)
    and is_oversea = 1 
