...
left join (
    ...
    select house_id
        ...
        ,cover_picture_url	
    from dws.dws_house_d
    where dt = concat(substr(current_date,7),'-','01')
    AND landlord_channel_name = '平台商户'
    and house_is_online = 1 
    AND house_is_oversea = 1
) h1 
...
left join (
    ...
    select unitid house_id 
        ,count(1) totalscore	
        ,round(count(distinct case when totalscore > 4 then comment_id end) / count(distinct case when totalscore > 0 then comment_id end),4) good_comment_rate
