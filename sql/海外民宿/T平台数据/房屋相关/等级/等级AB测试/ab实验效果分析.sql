...
where dt between '2025-06-03' and date_sub(current_date,1)
...
select a.*
    -- 'B','C','D','E','F','G','H','I','J','K'
    ,case when t2.bucket_type in ('D','E') then '实验'
        when a.dt >= '2025-06-10' and t2.bucket_type in ('F','G','H') then '实验'
        when a.dt <= '2025-06-09' and t2.bucket_type in ('F','G','H') then '对照'
        when t2.bucket_type in ('B','C','F','G','H','I','J','K') then '对照'
        else '其他'
        end as bucket
    ,t2.bucket_type
    ,source_type
