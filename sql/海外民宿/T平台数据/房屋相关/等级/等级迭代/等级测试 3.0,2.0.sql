...
--诚信分
    ...
    -- 只改动c接
        when city_level = 'S级' and landlord_channel = 334 then 
    (nvl(t_rk_dynamic_gmv_score,0)*0.3+ nvl(c_rk_dynamic_gmv_score,0)*0.2 + nvl(comment_score,0)*0.1 + nvl(infor_score,0)*0.1 + nvl(house_quality_score,0)*0.15 + nvl(style_score,0)*0.15 + nvl(reward_score,0) + nvl(credit_score,0) + nvl(penalty_score,0))
        when landlord_channel = 1 then 
    ...
    -- 只改动c接
            when landlord_channel = 334 then 
    (nvl(t_rk_dynamic_gmv_score,0)*0.3+ nvl(c_rk_dynamic_gmv_score,0)*0.2 + nvl(comment_score,0)*0.1 + nvl(infor_score,0)*0.1 + nvl(house_quality_score,0)*0.15 + nvl(style_score,0)*0.15 + nvl(reward_score,0) + nvl(credit_score,0) + nvl(penalty_score,0) + nvl(house_type_jiudian_score,0))
    else 
    (nvl(t_rk_dynamic_gmv_score,0)*0.3+ nvl(c_rk_dynamic_gmv_score,0)*0.2 + nvl(comment_score,0)*0.1 + nvl(infor_score,0)*0.1 + nvl(house_quality_score,0)*0.15 + nvl(style_score,0)*0.15 + nvl(reward_score,0) + nvl(credit_score,0) + nvl(penalty_score,0) + nvl(house_type_jiudian_score,0))
    end as House_Score
