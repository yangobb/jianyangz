...
END AS is_new_house, --30天新房
    ...
    picture_count, -- 260112 图片数量
    hygiene_comment_score -- 260112 房屋卫生评论分
...
case when city_name in ('首尔') and (house_type = '青旅' OR
      ...
      house_name LIKE '%背包客%') then '0'--20250818 修正比分--2025-02-14修改青旅惩罚分降低原-2.0
    when city_name not in ('首尔') and (house_type = '青旅' OR
      house_name LIKE '%睡眠报告%' OR
      house_name LIKE '%睡眠舱%' OR
      house_name LIKE '%胶囊%' OR
      house_name LIKE '%男性%' OR
      ...
      house_name LIKE '%青年%' OR
      house_name LIKE '%宿舍%' OR
      house_name LIKE '%背包客%') then '-1.0'--20250818 修正比分--2025-02-14修改青旅惩罚分降低原-2.0
...
,all_score_info as (select distinct  
    ...
    a.picture_count, -- 260112 图片数量
    a.hygiene_comment_score, -- 260112 房屋卫生评论分
    comment_num,
...
--诚信分
    ...
    ,city_level_yunying
    ,case when yx.house_id is not null then 1 else 0 end is_youxuan
    ,case when bz.house_id is not null then 1 else 0 end is_baozang
    ,case when sp.house_id is not null then 1 else 0 end is_video
    ,'' is_c_standard
    ,picture_count -- 260112 图片数量
    ,hygiene_comment_score -- 260112 房屋卫生评论分
    ,comment_num -- 260112 评论数量
...
left join (
    select house_id from pdb_analysis_b.dwd_house_label_1000487_d where dt = date_sub(current_date,2)
) yx on a.house_id = yx.house_id
left join (
    select house_id from pdb_analysis_b.dwd_house_label_1000488_d where dt = date_sub(current_date,2)
) bz on a.house_id = bz.house_id
left join (
    select house_id from pdb_analysis_b.dwd_house_label_1000293_d where dt = date_sub(current_date,2)
) sp on a.house_id = sp.house_id
left join (
    select city_name,city_level city_level_yunying from excel_upload.oversea_city_level
) cs on a.house_city_name = cs.city_name
...
city_level_yunying     -- 260112 城市运营等级
,is_youxuan            -- 260112 优选
,is_baozang            -- 260112 宝藏
,is_video              -- 260112 真视频
,is_c_standard         -- 260112 携程标准
,picture_count         -- 260112 图片数量
,hygiene_comment_score -- 260112 房屋卫生评论分
,comment_num           -- 260112 评论数量
