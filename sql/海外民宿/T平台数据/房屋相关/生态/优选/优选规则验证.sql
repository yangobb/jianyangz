...
from (
    select distinct 
        ...
        h.dynamic_business,
        h.`是否命中青旅关键词`,
        nvl(independentbathroom,0) `独立卫浴`,
        case when CAST(enum_house_facilities_name AS STRING) LIKE '%可洗热水澡%' THEN 1 ELSE 0 END `可洗热水澡`
    from (
        select *
            -- 周边有饭店
            ...
            ,case when regexp_like(house_name, '青旅|其他类型|睡眠报告|睡眠舱|胶囊|男性|女性|男女|混合|床位|青年|宿舍|背包客') = 1 then 1 else 0 end `是否命中青旅关键词`
        from dws.dws_house_d 
        where dt = date_sub("${date}",1) 
        and house_is_oversea = 1 
        and house_is_online = 1 
...
select
    a.*
    ,cast(rst_prob as double) rst_prob
    ,case when black.house_id is not null then 1 else 0 end `是否黑名单` 
    ,case when b.house_id is not null then 1 else 0 end `优选上标`
from final_house a
...
left join (
    --2024-12-11为全量算法打标分区,12号及以后为每天图片增量打标分区
    select *
    from (
        select
            house_id
            ,rst
            ,row_number() over(partition by house_id order by dt desc) as rn
            ,rst_prob
        from dw_algorithm.ads_house_hw_571_white_list_algorithm_results_d
        where dt >= "2024-12-14"
    ) a
    where rn = 1
) c 
