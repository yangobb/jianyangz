...
from (
    select  
        ...
        ,concat(round(`民宿离店间夜`*100/`酒店离店间夜`,1),'%')       as `离店间夜非标占比`
        ,concat(round(`民宿离店gmv`*100/`酒店离店gmv`,1),'%')         as `离店GMV非标占比`
        ,null                                                         as `酒店详情页uv`
        ,null                                                         as `酒店7大类详情页uv`
        ,null                                                         as `民宿详情页uv` 
        ,row_number() over(partition by `week` order by `酒店离店间夜` desc) -2 as `酒店离店间夜排名`
    from (
        select 
            ...
            ,sum(`酒店离店单`) as `酒店离店单`
            ,sum(`酒店离店间夜`) as `酒店离店间夜`
            ,sum(`酒店离店gmv`) as `酒店离店gmv`
            ,sum(`酒店七大类离店单`) as `酒店七大类离店单`
            ,sum(`酒店七大类离店间夜`) as`酒店七大类离店间夜`
            ,sum(`酒店七大类离店gmv`) as`酒店七大类离店gmv`
            ,sum(`民宿离店单`) as `民宿离店单`
            ,sum(`民宿离店间夜`) as `民宿离店间夜`
            ,sum(`民宿离店gmv`) as `民宿离店gmv`
        from jdm 
        group by 1,2,3 
    ) h
) h1
where `酒店离店间夜排名`<=50
order by `日期范围` desc,`酒店离店间夜排名`
