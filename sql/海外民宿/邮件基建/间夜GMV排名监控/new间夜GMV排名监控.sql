...
with all_data as (
    select sum(order_room_night_count) as night,
        ...
        from (select order_room_night_count 
                ,room_total_amount
                ,sell_channel
                ,sell_channel_type
                ,api_sell_channel_type,create_date
                ,is_success_order,is_overseas
                ,landlord_source_channel_code
...
from (
    select checkout_date create_date
        ,count(distinct ord_no) order_cnt 
        ,sum(gmv) gmv
        ,sum(rn_cnt_cii) night
    from app_ctrip.v_edw_inpr_aa_ovs_ord_d
    where d = date_sub(current_date,1)
    and checkout_date >= date_add(current_date,-14) 
...
left join (
    ...
    from app_ctrip.v_edw_inpr_aa_ovs_ord_d
    where d = date_sub(current_date,1)
    and checkout_date >= add_months(date_add(current_date,-16),-12)
    and checkout_date < add_months(date_add(current_date,2),-12)
    and ord_status in ('P','S')
    and is_tcom = 0
    group by 1 
) d 
...
select concat(a.create_date,'(',c.dayofweek ,')')  create_date
    ...
    ,round(sum(d.gmv),1) as gmv_ly
from (
    select bk_date create_date
        ,count(distinct ord_no) order_cnt 
        ,sum(gmv) gmv
        ,sum(rn_cnt_cii) night
    from app_ctrip.v_edw_inpr_aa_ovs_ord_d
    where d = date_sub(current_date,1)
    and bk_date >= date_add(current_date,-14)
    and bk_date < current_date
    and ord_status in ('P','S')
    and is_tcom = 0
    group by 1 
) a 
...
left join (
    ...
    from app_ctrip.v_edw_inpr_aa_ovs_ord_d
    where d = date_sub(current_date,1)
    and bk_date >= add_months(date_add(current_date,-16),-12)
    and bk_date < add_months(date_add(current_date,2),-12)
    and ord_status in ('P','S')
    and is_tcom = 0
    group by 1 
) d 
