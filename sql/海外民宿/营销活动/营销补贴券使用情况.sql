...
where
    ...
    and activity_name regexp '错峰出游红包|国庆早订券|连住专享券|暑期连住券|惊喜红包|810福利券|惊喜早鸟特惠|房东感恩红包|国庆复购券|国庆连住券|国庆贵宾礼遇'
    and payment_provider = 1
) q --使用24年暑期连住券
  on o.order_no = q.order_no  
  where o.is_success_order=1 --支付成功
   and order_status<>'CANCELLED' --非取消
   and o.create_date >= '2025-07-18'  --下单日期
--   and checkin_date between '2025-07-18' and '2025-08-31'
  group by 1,2
),
...
,case 
        ...
        when activity_code in ('rp_CN1gLosJaeA','rp_CN1g2wZgTSY','rp_CNOny96VA8O','rp_CNfYn3LqOIO','rp_CNfMkAIAbec','rp_CZ7fC9OlR5W','rp_CZ7EVa7aD6E','rp_CZ7Go66Bvm6','rp_CZ7POcQLvIi','rp_CZmjnvgMjHW','rp_CZmFnrGbVfa',
        'rp_CZmVd5hcvz4','rp_CZmWLimvgeU','rp_CZqdL88gVu2','rp_CZqaDANxy5S','rp_CZqDNryMGg0','rp_CZqDksd8kC4','rp_CZqD4IsWHMy','rp_CZqAcKeaumc','rp_CZoLA9ONQWU','rp_CZoXmPIlGec','rp_CLvKqHICdij','rp_CLvK2BYUpTi',
        'rp_CL2U7ZRXu4c',"rp_CL2k7X60Ohq","rp_CL2yckXdm5m",'rp_CZf2Tr1ixcA','rp_CZJFz6Lnlc0','rp_CZJFbdMzJ77','rp_CZJc0f02nVe','rp_CZjIYUF4FNm','rp_CZAIWRn2Uuq','rp_CZAivbfV4dq','rp_COLC6tvCXl4','rp_COLBh5racEj','rp_COLfEm00tvF') then '连住专享券'
...
and to_date(create_time) between '2025-07-18' and date_sub(current_date,1)
group by 1,2
),
...
left join
red_fafang p
on p.dt=pr.create_date and p.red_name = pr.activity_name
left join all_order_list o on pr.create_date = o.create_date
),
...
where `领券日期` between date_sub(current_date,7) and date_sub(current_date,1)
order by `领券日期` desc
) a
) mm 

order by `红包名称`,`领券日期` desc
